...
def compute_token_accuracy(generated: str, target: str) -> float:
    ...
    if not gen_tokens:
        return 0.0
    matches = sum(1 for pred, ref in zip(gen_tokens, targ_tokens) if pred == ref)
...
def evaluate(model: str = DEFAULT_MODEL, max_samples: int = 100, use_random: bool = True) -> dict:
    ...
    for index, (nl, target) in enumerate(test_data, start=1):
        ...
        if not validate_regex(generated):
            invalid_regexes += 1
        accuracy = compute_token_accuracy(generated, target)
    ...
    return {
        ...
        "valid": valid,
        "token_accuracy": sum(accuracies) / len(accuracies) if accuracies else 0.0,
    }
...
def print_results(results: dict) -> None:
    ...
    print(f"Average token accuracy: {results['token_accuracy']:.4f}")
    print("==============================================")
